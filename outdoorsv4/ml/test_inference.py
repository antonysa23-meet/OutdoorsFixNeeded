...
Run: python outdoorsv4/ml/test_inference.py
"""
...
def test_phase_a():
    ...
    for prompt, expected_intent, expected_formats, desc in PHASE_A_TESTS:
        ...
        print(f"         Intent:  {actual_intent} (expected {expected_intent}) {'OK' if intent_ok else 'X'}")
        if expected_formats is not None:
            print(f"         Formats: {actual_formats} (expected {expected_formats}) {'OK' if format_ok else 'X'}")
        else:
